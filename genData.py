...
###################################################################################################
def main():
    imgTrainingNumbers = cv2.imread("train_images/train_data_ten_fonts.png")            # read in training numbers image
    ...
    '''img = cv2.GaussianBlur(imgTrainingNumbers, (3, 3), 0)
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (11, 11))
    close = cv2.morphologyEx(gray, cv2.MORPH_CLOSE, kernel)
    div = np.float32(gray) / (close)
    imgBlurred= np.uint8(cv2.normalize(div, div, 0, 255, cv2.NORM_MINMAX))
    '''
